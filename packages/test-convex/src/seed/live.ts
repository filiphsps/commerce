...
export async function seedCanonicalLive(url: string, opts: SeedCanonicalOptions = {}): Promise<string> {
    ...
    // Collaborators: import the users (each carrying its embedded OAuth identity), then link them to
    // the shop. The link rides through `upsertShop`'s `(shop, user)`-idempotent collaborator sync.
