...
import { MEDIA_MIME_TYPES } from './mime-types';
 ...
 * It is a mirror rather than an import because this package is off the Convex bundle surface — and the
 * Convex package's `tsc` program deliberately excludes node types, so the guard lives HERE
 * (node-typed tests, real import of the source of truth) and reads the mirror's source from disk.
 */
