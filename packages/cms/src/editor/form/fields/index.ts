 ...
 * {@link registerCompositeFieldWidgets} (group/array/collapsible). The
...
/**
 * Register the three composite container widgets — `group`, `array`, and the
 * presentational `collapsible` — into a field registry. Registration is
 * last-write-wins, so a host may override any of these afterward. Returns the
 * same registry for call-site chaining. The widgets recurse through
 * {@link RenderFields}, so a registry carrying both the scalar and composite
 * widgets renders an arbitrarily deep descriptor tree.
 *
 * @param registry - The registry to populate.
