...
This guide is the step-by-step for hooking up a new CMS collection to the
admin editor system. Every editable collection follows this shape — there is
no bespoke-route alternative.

## Prerequisites

- The slug is a member of the `CollectionSlug` union
  (`packages/cms/src/editor/manifest.ts`).
- The collection's field descriptors and drafts/autosave behavior are declared
  in `editorCollectionSchema` (`packages/cms/src/editor/collection-fields.ts`).
