...
2. `resolveBuildId` determines that value using this precedence:
   `NEXT_DEPLOYMENT_ID` → `VERCEL_DEPLOYMENT_ID` → `GIT_COMMIT_SHA` → `NEXT_BUILD_ID` (auto-generated by Next.js) → a UUID fallback.
...
The route is also annotated `force-dynamic` and responds with `Cache-Control: no-store` so Vercel's CDN never serves a stale response.
