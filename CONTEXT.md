...
**Block**:
A discriminated-union content unit defined in the **CMS**. Some block types project Shopify data and require a **Block loader**. Page content the editor *places* — distinct from a **Section** (fixed page chrome) and from a configurable **Component** (ambient UI, never placed). A Block carries per-instance content (`fields`, edited per node in the page editor) and optionally store-wide defaults (`settings`, edited in the **Customization hub**'s Blocks tab); a Block may render **Components** inside it. Editor-placeability is the line: an editor drops Block instances onto a page, never a Component.
_Avoid_: section (now a distinct term — see **Section**), content block, widget
...
**Block availability**:
A per-**Shop** restriction (**Shop extension manifest** `extensions.blocks.available`) on which **Block** types exist for the shop — a shop-level capability switch, not a per-page choice. Absent = every type available; an explicit subset restricts. Restricting a type both removes it from the editor's add menu AND stops already-placed instances of that type rendering (the **Storefront** dispatcher skips unavailable types). Re-allowing every type returns to the inherit/absent state.
_Avoid_: block visibility, block toggle

**Block setting**:
A configurable presentation/behavior knob a **Block** type declares (`BlockDescriptor.settings[]`, e.g. the collection block's `defaultLayout`), `overridable`, resolved through the **Setting cascade** (no per-**Surface** tier). The *same* descriptor set is authored in two places: store-wide in the **Customization hub**'s Blocks tab (its store-wide tier value — the block's *default* — stored at `extensions.blockDefaults.<slug>.<name>`) and per-instance in the page editor's "Overrides" group (stored on the **Block** node). Distinct from a Block's content **fields** (per-instance authored data — title, handle); the two can coexist on one node (the collection block's content `layout` wins over its `defaultLayout` setting). Parallel to a **Component setting** but block-scoped, not surface-scoped.
_Avoid_: block field, block config, block option, block default (use "the block's store-wide default")

**Shop extension manifest**:
An optional, declarative per-**Shop** config (`ShopExtensionManifest`, `@nordcom/commerce-cms/extensions`) that UNIFIES — never forks — the per-shop surfaces `resolveExtensions` composes: theme tokens (`resolveTheme`), **Section** order and visibility (`resolveChromeLayout`), available **Block** types (`BLOCK_TYPES` / `isBlockType`), and a **Component setting** registry of store-wide defaults (product-card variant selections, per-block defaults, and the build-notifier banner config). Its role is shop-level enable/disable of features, extensions, and plugins — operator-owned and coarse, NOT a rollout/targeting mechanism (that is the **Feature flag** system); where both gate one knob (a **Section**'s visibility) the manifest wins. The registry is extensible — a new configurable component appends a `COMPONENT_SETTINGS` entry plus a `ResolvedExtensions` field, not a new manifest concept; don't hard-count the surfaces. CMS-safe — the type and its pure `resolveExtensions` composer import only the db theme leaf, the errors package, and CMS-internal schemas; never React, Shopify, the **Storefront**, or a **Provider token** — so the **Block loader** firewall holds. An absent or empty manifest composes byte-identically to today's render.
...
**Section**:
A toggleable region of the storefront page chrome — `info-bar`, `header`, `footer`; the body `content` region is required and never toggleable. Same region a **chrome slot** names from the layout-order angle (`ChromeSlotId`, `resolveChromeLayout`): "Section" is the visibility/operator-facing name (shown vs hidden, edited in the Customization hub's Sections tab), "chrome slot" the ordering/registry name. Visibility resolves bottom-up: hardcoded default (shown) → a `section:<id>` **Feature flag** (the platform's rollout/targeting layer) → the per-**Shop** **Shop extension manifest** override (`extensions.sections.<id>`, the operator's shop-level on/off — wins, short-circuiting the flag). Order resolves through the same `resolveChromeLayout`. NOT a **Block** — a Section is fixed page chrome, a Block is editor-added page content.
_Avoid_: chrome region, page section, block (see **Block**)

**Customization hub**:
The **Admin** surface where an operator edits a **Shop**'s customization — one tabbed editor (deep-linked via `?tab=`) over Theme (**Theme tokens**), Components (**Component settings**), Blocks (**Block** store-wide defaults), and Sections (**Section** visibility + **Block availability**). Mounts inside a single editor form so the toolbar saves and publishes every tab together; all of it writes the **Shop extension manifest** (plus the theme leaf). Edits the **Shop** as a draft/published, versioned CMS document (Save Draft → Publish, restore via versions) — so customization is staged and previewed in the editor and reaches the **Storefront** only on Publish, not live-on-save.
_Avoid_: customization settings, shop settings, theme editor (now just the hub's Theme tab)
...
**Feature flag**:
A globally-defined named toggle with a `key`, `defaultValue`, optional `options[]`, and `targeting[]` rules, built on the Vercel Flags SDK (`flags/next`). A **Shop** opts in by storing a ref to a flag; evaluation walks the flag's targeting predicates — `percentage` (rollouts bucketed by a per-visitor `visitorId`), `shop`, `authenticated`, `group`, `always` — and Vercel Toolbar overrides apply on the dynamic (non-cached) path. The rollout/targeting/experiment layer. Distinct from a **Component setting** (per-shop component config, no key/targeting). Distinct from the **Shop extension manifest**: a flag rolls out / targets / experiments; the manifest enables/disables features at the shop level. Where both gate one knob (a **Section**'s visibility), the manifest's shop-level switch wins over the flag.
