...
Storefront UI lives in `apps/storefront/src/components/` (e.g. `product-card`, `product-options`, `typography`, `layout`). Shared, cross-app code lives in `packages/`. There is no single central token file — confirm the target design-system location in Step 1 before extracting.

## Step 1: Discover the Design System

Find the design system, component library, or shared UI directory. Understand its structure: component organization, naming conventions, design token structure, import/export conventions.

**CRITICAL**: If no design system exists, ask the user (use the `AskUserQuestion` tool) before creating one. Understand the preferred location and structure first.
