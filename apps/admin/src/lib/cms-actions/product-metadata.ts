...
import type { ProductMetadatum } from '@nordcom/commerce-cms/types';
...
/**
 * Subset of the productMetadata collection fields that the form can supply.
 *
 ...
 */
type ProductMetadataInput = Pick<ProductMetadatum, 'descriptionOverride' | 'blocks' | 'seo'>;
 ...
 */
function parseFormData(formData: FormData): Partial<ProductMetadataInput> {
    ...
    try {
        ...
        // an empty doc.
        console.error('[product-metadata] Failed to parse _payload JSON', err);
...
/**
 * Upserts a productMetadata doc keyed by (tenant, handle).
 *
...
async function upsert(domain: string, handle: string, formData: FormData, status: MetadataStatus): Promise<void> {
    ...
    if (!tenant) {
        console.error('[product-metadata] upsert called without a resolved tenant');
        notFound();
    }
    ...
    revalidatePath(`/${domain}/content/product-metadata/`);
    revalidatePath(`/${domain}/content/product-metadata/${handle}/`);
}
...
/**
 * Saves the product metadata doc as a draft (create or update).
 *
 * `domain` and `handle` are bound positional args from RSC:
 * `saveProductMetadataDraftAction.bind(null, domain, handle)`.
 * The client component calls `saveDraftAction(formData)` after binding.
 */
export async function saveProductMetadataDraftAction(
    domain: string,
    handle: string,
    formData: FormData,
): Promise<void> {
