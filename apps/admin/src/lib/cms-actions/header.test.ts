...
import { publishHeaderAction, saveHeaderDraftAction } from './header';
...
describe('saveHeaderDraftAction', () => {
    beforeEach(() => {
    ...
    it('creates a new doc with _status: draft when no existing doc is found', async () => {
        ...
        expect(mockRevalidatePath).toHaveBeenCalledWith(`/${DOMAIN}/content/header/`);
    });
    ...
    it('updates the existing doc with _status: draft when an existing doc is found', async () => {
        ...
        const formData = makeFormData({ logoLink: '/updated' });
        await saveHeaderDraftAction(DOMAIN, formData);
...
describe('publishHeaderAction', () => {
    ...
    it('creates a new doc with _status: published when no existing doc is found', async () => {
        ...
        const formData = makeFormData({ logoLink: '/' });
        await publishHeaderAction(DOMAIN, formData);
    ...
    it('updates the existing doc with _status: published when an existing doc is found', async () => {
        ...
        const formData = makeFormData({ logoLink: '/final' });
        await publishHeaderAction(DOMAIN, formData);
...
describe('FormData parsing (_payload JSON blob)', () => {
    ...
    it('parses nested fields correctly from the _payload JSON blob', async () => {
        ...
        const formData = makeFormData({
            ...
            localeSwitcher: { enabled: true, label: 'Language' },
            cta: { kind: 'external', label: 'Get started', url: '/start' },
        });
            ...
            expect.objectContaining({
                data: expect.objectContaining({
                    ...
                    items: expect.arrayContaining([
                        expect.objectContaining({ link: expect.objectContaining({ label: 'Shop' }) }),
                    ]),
                    localeSwitcher: expect.objectContaining({ enabled: true }),
                    cta: expect.objectContaining({ kind: 'external', label: 'Get started' }),
                }),
            }),
        );
    });
    ...
    it('throws when _payload contains invalid JSON (no payload call, surfaces error to <Form>)', async () => {
        ...
        try {
            await expect(saveHeaderDraftAction(DOMAIN, formData)).rejects.toThrow('Malformed form payload');
        } finally {
...
describe('Tenant scoping and security', () => {
    ...
    it('uses tenant from getAuthedPayloadCtx, not from FormData (cross-tenant forgery prevention)', async () => {
        ...
        const formData = makeFormData({ logoLink: '/', tenant: 'evil-tenant-999' });
        await saveHeaderDraftAction(DOMAIN, formData);
    ...
    it('forwards user to every payload call so access checks run correctly', async () => {
        ...
        const formData = makeFormData({ logoLink: '/test' });
        await publishHeaderAction(DOMAIN, formData);
