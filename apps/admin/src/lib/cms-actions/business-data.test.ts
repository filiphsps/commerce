...
import { publishBusinessDataAction, saveBusinessDataDraftAction } from './business-data';
...
describe('saveBusinessDataDraftAction', () => {
    beforeEach(() => {
    ...
    it('creates a new doc with _status: draft when no existing doc is found', async () => {
        ...
        const formData = makeFormData({ legalName: 'Acme Corp', supportEmail: 'support@acme.io' });
    ...
    it('updates the existing doc with _status: draft when an existing doc is found', async () => {
        ...
        const formData = makeFormData({ legalName: 'Updated Name' });
        await saveBusinessDataDraftAction(DOMAIN, formData);
...
describe('publishBusinessDataAction', () => {
    beforeEach(() => {
        mockGetAuthedPayloadCtx.mockReset();
        mockRevalidatePath.mockReset();
    });
    ...
    it('creates a new doc with _status: published when no existing doc is found', async () => {
        ...
        const formData = makeFormData({ legalName: 'Acme Corp' });
        await publishBusinessDataAction(DOMAIN, formData);
    ...
    it('updates the existing doc with _status: published when an existing doc is found', async () => {
        ...
        const formData = makeFormData({ legalName: 'Final Name' });
        await publishBusinessDataAction(DOMAIN, formData);
    ...
    it('parses nested fields correctly from the _payload JSON blob', async () => {
        ...
        expect(payload.create).toHaveBeenCalledWith(
            expect.objectContaining({
                data: expect.objectContaining({
                    legalName: 'Acme',
                    address: expect.objectContaining({ line1: '123 Main St', city: 'Springfield' }),
                    profiles: expect.arrayContaining([
                        expect.objectContaining({ platform: 'instagram', handle: '@acme' }),
                    ]),
                }),
            }),
        );
...
describe('Tenant scoping and security', () => {
    ...
    it('uses tenant from getAuthedPayloadCtx, not from FormData (cross-tenant forgery prevention)', async () => {
        ...
        const formData = makeFormData({ legalName: 'Injected', tenant: 'evil-tenant-999' });
        await saveBusinessDataDraftAction(DOMAIN, formData);
    ...
    it('forwards user to every payload call so access checks run correctly', async () => {
        ...
        const formData = makeFormData({ legalName: 'Test' });
        await publishBusinessDataAction(DOMAIN, formData);
