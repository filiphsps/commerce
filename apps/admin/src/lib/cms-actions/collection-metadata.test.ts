...
import { publishCollectionMetadataAction, saveCollectionMetadataDraftAction } from './collection-metadata';
...
function makePayload({
    ...
    createResult = { id: 'col-doc-new' } as unknown,
}: {
    existingDoc?: unknown;
...
// ------------------------------------------------------------------
// saveCollectionMetadataDraftAction — update path (existing doc)
// ------------------------------------------------------------------

describe('saveCollectionMetadataDraftAction — update (existing doc)', () => {
    beforeEach(() => {
    ...
    it('calls find by (tenant, handle) then update with _status: draft and revalidates', async () => {
        ...
        const formData = makeFormData({ seo: { title: 'Summer Sale' } });
        await saveCollectionMetadataDraftAction(DOMAIN, HANDLE, formData);
        ...
        expect(mockRevalidatePath).toHaveBeenCalledWith(`/${DOMAIN}/content/collection-metadata/`);
        expect(mockRevalidatePath).toHaveBeenCalledWith(`/${DOMAIN}/content/collection-metadata/${HANDLE}/`);
    });
    ...
    it('shopifyHandle is NOT included in the update data (never from FormData or update spread)', async () => {
        ...
        const formData = makeFormData({ shopifyHandle: 'evil-other-collection', seo: { title: 'X' } });
        await saveCollectionMetadataDraftAction(DOMAIN, HANDLE, formData);
...
// ------------------------------------------------------------------
// saveCollectionMetadataDraftAction — create path (no existing doc)
// ------------------------------------------------------------------

describe('saveCollectionMetadataDraftAction — create (no existing doc)', () => {
    beforeEach(() => {
        mockGetAuthedPayloadCtx.mockReset();
        mockRevalidatePath.mockReset();
        mockNotFound.mockReset().mockImplementation((): never => {
    ...
    it('creates doc when find returns empty, using handle from URL and tenant from ctx', async () => {
        const payload = makePayload({ existingDoc: null, createResult: { id: 'col-doc-new-1' } });
        mockGetAuthedPayloadCtx.mockResolvedValue(makeCtx(payload));

        const formData = makeFormData({ seo: { title: 'Summer Sale Collection' } });
        await saveCollectionMetadataDraftAction(DOMAIN, HANDLE, formData);
    ...
    it('shopifyHandle in create always comes from URL parameter, ignores FormData shopifyHandle', async () => {
        const payload = makePayload({ existingDoc: null, createResult: { id: 'col-doc-new-2' } });
        mockGetAuthedPayloadCtx.mockResolvedValue(makeCtx(payload));

        // Attacker injects a different handle via form
        const formData = makeFormData({ shopifyHandle: 'attacker-collection' });
        await saveCollectionMetadataDraftAction(DOMAIN, HANDLE, formData);
        ...
        expect(createCall.data.shopifyHandle).not.toBe('attacker-collection');
    });
    ...
    it('calls notFound when tenant is null', async () => {
        ...
        try {
            await expect(saveCollectionMetadataDraftAction(DOMAIN, HANDLE, new FormData())).rejects.toThrow(
                'NEXT_NOT_FOUND',
            );
        } finally {
...
describe('publishCollectionMetadataAction', () => {
    beforeEach(() => {
        mockGetAuthedPayloadCtx.mockReset();
        mockRevalidatePath.mockReset();
        mockNotFound.mockReset().mockImplementation((): never => {
    ...
    it('updates existing doc with _status: published', async () => {
        ...
        const formData = makeFormData({ seo: { title: 'Published Collection' } });
        await publishCollectionMetadataAction(DOMAIN, HANDLE, formData);
    ...
    it('creates doc with _status: published when no existing doc', async () => {
        const payload = makePayload({ existingDoc: null, createResult: { id: 'col-doc-pub-new' } });
        mockGetAuthedPayloadCtx.mockResolvedValue(makeCtx(payload));
...
describe('FormData parsing (_payload JSON blob)', () => {
    ...
    it('parses blocks and seo from the _payload JSON blob', async () => {
        ...
        const formData = makeFormData({
            blocks: [{ blockType: 'banner', heading: 'Summer Sale!' }],
            seo: { title: 'Collection SEO', description: 'Best deals', noindex: false },
        });
        await saveCollectionMetadataDraftAction(DOMAIN, HANDLE, formData);
        ...
        expect(payload.update).toHaveBeenCalledWith(
            expect.objectContaining({
                data: expect.objectContaining({
                    blocks: expect.arrayContaining([expect.objectContaining({ blockType: 'banner' })]),
                    seo: expect.objectContaining({ title: 'Collection SEO', description: 'Best deals' }),
                }),
            }),
        );
    });
    ...
    it('throws when _payload contains invalid JSON', async () => {
        ...
        const consoleErrSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
        try {
            await expect(saveCollectionMetadataDraftAction(DOMAIN, HANDLE, formData)).rejects.toThrow(
    ...
    it('_status is always server-determined, never from FormData', async () => {
        const payload = makePayload({ existingDoc: null, createResult: { id: 'col-doc-x' } });
        mockGetAuthedPayloadCtx.mockResolvedValue(makeCtx(payload));
        ...
        const createCall = payload.create.mock.calls[0]?.[0] as { data: Record<string, unknown> };
        // The action hardcodes 'draft' for saveCollectionMetadataDraftAction
