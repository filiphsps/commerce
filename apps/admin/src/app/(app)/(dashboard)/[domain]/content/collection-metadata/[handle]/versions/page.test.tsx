...
import CollectionMetadataVersionsPage, { metadata } from './page';

describe('CollectionMetadataVersionsPage', () => {
    it('passes the manifest + runtime + handle-as-id to <EditorVersionsPage>', async () => {
        const el = await CollectionMetadataVersionsPage({
    ...
    it('exposes the page title metadata', () => {
        expect(metadata.title).toBe('Collection metadata — Versions');
    });
});
