...
import ProductMetadataListPage from './page';
...
describe('(dashboard)/[domain]/content/product-metadata/page', () => {
    ...
    it('is an async function (server component)', () => {
        expect(typeof ProductMetadataListPage).toBe('function');
    });

    it('returns null when there is no tenant', async () => {
        mockGetAuthedCmsCtx.mockResolvedValue({ ...makeCtx(), tenant: null });

        const result = await ProductMetadataListPage({ params: validParams });
    ...
    it('renders the heading and the empty-state message when no docs exist', async () => {
        ...
        const { container } = await renderRSC(() => ProductMetadataListPage({ params: validParams }));
        const q = within(container as HTMLElement);

        expect(q.getByText('Product Metadata')).toBeInTheDocument();
        expect(
            q.getByText('No product metadata yet. Enter a Shopify handle above to create the first entry.'),
        ).toBeInTheDocument();
    });

    it('lists the productMetadata collection with a page size of 100', async () => {
        mockGetAuthedCmsCtx.mockResolvedValue(makeCtx());
        mockList.mockResolvedValue(listPage([]));

        await renderRSC(() => ProductMetadataListPage({ params: validParams }));

        expect(mockList).toHaveBeenCalledWith({ collection: 'productMetadata', pageSize: 100 });
    });
    ...
    it('renders a row per doc, labeling it by the Shopify handle', async () => {
        ...
        mockList.mockResolvedValue(
            listPage([
                { documentId: 'doc-1', data: { shopifyHandle: 'cool-sneakers' }, status: 'published', updatedAt: 0 },
            ]),
        );
    ...
    it('exports metadata with title "Product Metadata"', async () => {
        const { metadata } = await import('./page');
        expect((metadata as { title?: string }).title).toBe('Product Metadata');
    });
});
