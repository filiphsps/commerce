...
export default async function HeaderVersionsPage({ params }: { params: Params }) {
    ...
    return (
        <div className="mx-auto max-w-3xl px-6 py-10">
            <header className="mb-8 flex items-center justify-between">
                <h1 className="font-semibold text-2xl">Header — Versions</h1>
                <nav>
                    <Link
                        href={`/${domain}/content/header/` as Route}
            ...
            {docs.length === 0 ? (
                ...
                <ul className="divide-y divide-gray-200 rounded-lg border border-gray-200">
                    {docs.map((v) => {
                        // v.version is the snapshot of the header doc at the time
                        ...
                        const restoreThis = restoreVersionAction.bind(null, domain, 'header', String(v.id));
