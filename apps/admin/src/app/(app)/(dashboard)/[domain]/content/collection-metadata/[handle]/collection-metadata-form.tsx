...
export type CollectionMetadataFormProps = {
    /**
    ...
    /**
     * Server action with domain and handle already bound:
     * `publishCollectionMetadataAction.bind(null, domain, handle)`.
     * Signature after binding: `(formData: FormData) => Promise<void>`.
     */
 ...
 *
 * collectionMetadata has autosave enabled at the collection level (interval: 2000ms),
 * so the `useAutosave` delay here mirrors that config.
 */
export function CollectionMetadataForm({ saveDraftAction, publishAction }: CollectionMetadataFormProps) {
