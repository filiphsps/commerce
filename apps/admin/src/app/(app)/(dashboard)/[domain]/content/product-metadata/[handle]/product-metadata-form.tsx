...
export type ProductMetadataFormProps = {
    /**
     * Server action with domain and handle already bound:
    ...
    /**
     * Server action with domain and handle already bound:
     * `publishProductMetadataAction.bind(null, domain, handle)`.
     * Signature after binding: `(formData: FormData) => Promise<void>`.
     */
 ...
 *
 * productMetadata has autosave enabled at the collection level (interval: 2000ms),
 * so the `useAutosave` delay here mirrors that config.
 */
export function ProductMetadataForm({ saveDraftAction, publishAction }: ProductMetadataFormProps) {
