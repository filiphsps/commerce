...
export default async function BusinessDataVersionsPage({ params }: { params: Params }) {
    ...
    return (
        <div className="mx-auto max-w-3xl px-6 py-10">
            <header className="mb-8 flex items-center justify-between">
                <h1 className="font-semibold text-2xl">Business data — Versions</h1>
                <nav>
                    <Link
                        href={`/${domain}/content/business-data/` as Route}
            ...
            {docs.length === 0 ? (
                ...
                <ul className="divide-y divide-gray-200 rounded-lg border border-gray-200">
                    {docs.map((v) => {
                        // v.version is the snapshot of the businessData doc at
                        // the time this version was saved. v.latest === true means
                        // this version IS the current published state, so restoring
                        // it would be a no-op — disable the button to avoid confusion.
