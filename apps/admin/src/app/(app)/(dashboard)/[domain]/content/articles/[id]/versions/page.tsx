...
export default async function ArticleVersionsPage({ params }: { params: Params }) {
    ...
    // We filter by BOTH `parent` (to scope to this specific article) AND
    ...
    const restoreArticleVersion = (versionId: string) => restoreVersionAction.bind(null, domain, 'articles', versionId);

    return (
        <div className="mx-auto max-w-3xl px-6 py-10">
            <header className="mb-8 flex items-center justify-between">
                <h1 className="font-semibold text-2xl">Article — Versions</h1>
                <nav>
                    <Link
                        href={`/${domain}/content/articles/${id}/` as Route}
            ...
            {docs.length === 0 ? (
                ...
                <ul className="divide-y divide-gray-200 rounded-lg border border-gray-200">
                    {docs.map((v) => {
                        // v.version is the snapshot of the article doc at the time
                        ...
                        const restoreThis = restoreArticleVersion(String(v.id));
