...
import { publishHeaderAction, saveHeaderDraftAction } from '@/lib/cms-actions/header';
...
export default async function HeaderPage({ params, searchParams }: HeaderPageProps) {
    ...
    const { state: initialState } = await buildFormState({
        ...
        // than passing `true`. For header all fields are uniformly writable so
    ...
    const boundSaveDraft = saveHeaderDraftAction.bind(null, domain);
    const boundPublish = publishHeaderAction.bind(null, domain);

    return (
        <DocumentForm
            title="Header"
            breadcrumbs={[{ label: 'Content', href: `/${domain}/content/` as Route }, { label: 'Header' }]}
            ...
            toolbar={
                <>
                    ...
                    <HeaderForm saveDraftAction={boundSaveDraft} publishAction={boundPublish} />
                </>
            }
