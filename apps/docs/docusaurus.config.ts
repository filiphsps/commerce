...
type SidebarItem = { type: string; label?: string };
 ...
 * `Api` category (created from the symlinked `<workspace>/docs/api` directory),
 * relabels it to `API Reference`, collapses it, and pins it to the end so it
 * appears below handwritten content.
 */
async function sidebarApiCategoryReorder({
    ...
    const idx = items.findIndex((it) => it.type === 'category' && it.label?.toLowerCase() === 'api');
