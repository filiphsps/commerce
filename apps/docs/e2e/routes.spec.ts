...
test.describe('Navigation', () => {
    ...
    test('no standalone API dropdown in navbar', async ({ page }) => {
        await page.goto('/commerce/docs/getting-started/');
        // The API content is merged into each workspace's sidebar, so the
        // navbar must not surface a top-level "API" dropdown.
        await expect(page.getByRole('navigation').getByRole('button', { name: /^API$/i })).toHaveCount(0);
    });
});

test.describe('Per-workspace sidebar', () => {
    test('db workspace sidebar contains "API Reference" category', async ({ page }) => {
        await page.goto('/commerce/docs/db/overview/');
        // The autogenerated sidebar surfaces the API category as a collapsible
        // button labelled "API Reference" (renamed from the default "Api" by
        // docsPlugin's sidebarItemsGenerator).
        await expect(page.getByRole('button', { name: /API Reference/i }).first()).toBeVisible();
    });

    test('storefront workspace sidebar has no "API Reference" category', async ({ page }) => {
        await page.goto('/commerce/docs/storefront/overview/');
        // storefront has no TypeDoc output today, so no api/ subdir is
        // populated and no API Reference category should appear.
        await expect(page.getByRole('button', { name: /API Reference/i })).toHaveCount(0);
    });
});
