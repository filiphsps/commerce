...
- `product` / `collection` / `blog` / `article` `generateMetadata` →
  `'days'` (or `'max'` where already in place; don't downgrade without
  good reason).
- `search` results metadata → `'minutes'` (intentional shorter TTL).
- Shop root (`[locale]/layout.tsx` `generateMetadata`) → `'max'`.
- `cart` and `account` page metadata are nominally cached at `'max'`
  because the *metadata* (titles, robots tags) is per-tenant, not
  per-user. The cart **contents** are client-only and never see Next's
  cache.
