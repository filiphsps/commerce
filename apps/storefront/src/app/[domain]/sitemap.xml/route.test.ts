...
function makeRequest(): Request {
    return new Request('http://staging.storefront.localhost/staging.storefront.localhost/sitemap.xml');
}
...
describe('app/[domain]/sitemap.xml', () => {
    describe('GET', () => {
        ...
        it('uses the shop domain as the base URL for all sitemap hrefs', async () => {
            ...
            const body = await res.text();
            expect(body).toContain('https://staging.storefront.localhost/sitemaps');
        });
    });
});
