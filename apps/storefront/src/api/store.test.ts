...
describe('api/store', () => {
    describe('BusinessDataApi', () => {
        ...
        it('recursion guard: stops after at most 2 calls when getSingle always throws NotFoundError', async () => {
            ...
            // The guard compares fallback.code with _locale derived from client.defaultParams.lang.
            ...
            const getSingleMock = vi.fn().mockRejectedValue(new NotFoundError('business_data not found'));
            ...
            await expect(BusinessDataApi({ shop, locale })).rejects.toMatchObject({
