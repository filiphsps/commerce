...
export function chipClassName({
    ...
    return cn(
        ...
        density === 'compact' ? 'min-h-8 px-(--block-padding) py-0.5' : 'min-h-12 px-(--block-padding) py-(--block-padding-small)',
        available && !selected && 'cursor-pointer motion-safe:hover:-translate-y-px motion-safe:hover:shadow-[0_4px_12px_-2px_color-mix(in_srgb,var(--color-dark)_12%,transparent)]',
        selected &&
