        ...
        <Dialog.Root open={open} onOpenChange={onOpenChange} modal={true}>
            <Dialog.Portal>
                ...
                <Dialog.Content asChild={true} {...(description ? {} : { 'aria-describedby': undefined })}>
                    <div className="fixed top-1/2 left-1/2 z-30 flex max-h-[80vh] w-[calc(100%-2rem)] max-w-md -translate-x-1/2 -translate-y-1/2 flex-col gap-3 overflow-y-auto rounded-lg border border-(--border-default) border-solid bg-(--surface-0) p-4 shadow-lg">
                        <header className="flex items-center justify-between gap-2">
                            ...
                            <Dialog.Close asChild={true}>
                                <Button
                                    aria-label="Close"
                                    title="Close"
                                    className="text-(color:var(--text-muted)) hover:text-(color:var(--text)) flex size-6 items-center justify-center transition-colors"
                                    styled={false}
                                >
