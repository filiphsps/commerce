...
export const Button = <ComponentGeneric extends ElementType = 'button'>({
    ...
    return (
        <Tag
            {...props}
            className={cn(
                'appearance-none transition-color duration-150',
                styled &&
                    'inline-flex max-h-full cursor-pointer select-none items-center justify-center gap-1 rounded-xl bg-primary px-3 py-2 font-semibold text-base text-primary-foreground leading-none transition-all duration-150 *:text-inherit disabled:cursor-not-allowed disabled:bg-gray-300 disabled:text-gray-500 disabled:shadow-none disabled:brightness-100 data-[success=true]:bg-green-600 data-[success=true]:text-white',
                !props.disabled &&
                    styled &&
