...
export function GeoRedirect({ countries, locale, shop, i18n: defaultI18n }: GeoRedirectProps) {
    ...
    const geoCountry = location.country;
    const targetCountry =
        geoCountry !== undefined
            ? countries.find(({ isoCode }) => isoCode.toLowerCase() === geoCountry.toLowerCase())
            : null;
    ...
    return (
        <div className="text-(color:var(--text)) sticky inset-x-0 bottom-0 z-50 w-full border-(--border-strong) border-0 border-b-2 border-solid bg-(--surface-1) px-2 py-3 transition-all md:px-2">
                ...
                <div className="flex w-full grow gap-3 md:w-96">
                    ...
                    >
                        <button
                            ...
                            type="button"
                            className={cn(
                                'flex h-10 w-full grow cursor-pointer select-none appearance-none items-center justify-start gap-3 rounded-lg p-2 text-base leading-none *:select-none focus-within:bg-(--surface-1) hover:bg-(--surface-1)',
